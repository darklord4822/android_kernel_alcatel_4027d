...
static struct i2c_board_info __initdata kxtj2_i2c_board_info  = {	I2C_BOARD_INFO(KXTJ2_DEV_NAME, KXTJ2_I2C_SLAVE_ADDR>>1)};
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_SetDataResolution(struct kxtj2_i2c_data *obj , u8 data_res)
{
	int err;
	u8  databuf[2], reso;
	...
	databuf[0] &= ~KXTJ2__RESOLUTION_MASK;
	databuf[0] &= 0XF8;                             ////disable wakeup function
	databuf[0] |= data_res;							//
	databuf[1]  = databuf[0];
...
static int KXTJ2_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	return KXTJ2_SetDataResolution(obj,KXTJ2__RESOLUTION_12BIT);    	//set 12bit resolution
}
/*----------------------------------------------------------------------------*/
...
//////verify the config value correctly 
	hwmsen_read_block(client, KXTJ2_REG_DATA_FORMAT, &verify_reg_temp, 0x01);
	verify_reg_value[0]=verify_reg_temp;
	hwmsen_read_block(client, KXTJ2_REG_BW_RATE, &verify_reg_temp, 0x01);
	verify_reg_value[1]=verify_reg_temp;
	hwmsen_read_block(client, KXTJ2_REG_INT_ENABLE, &verify_reg_temp, 0x01);
	verify_reg_value[2]=verify_reg_temp;

	printk("the config register value is 0X1B=%x ,0x1E=%x ,0x21=%x \n",verify_reg_value[0],verify_reg_value[1],verify_reg_value[2]);
///////the correct value of 0x1b=0xc0 , 0x1e=0x00 , 0x21=KXTJ2_BW_100HZ;	

////solution of resume error
	if(((verify_reg_value[0]&0xD8) != (KXTJ2__RESOLUTION_12BIT|KXTJ2_RANGE_2G|KXTJ2_MEASURE_MODE))||
		((verify_reg_value[1]&0x0F) != KXTJ2_BW_100HZ)||(verify_reg_value[2] != 0x00))
	{
		reply_cnt++;
		if(reply_cnt<REPLY_MAX)
		{
			mdelay(5);
			goto set_config;
		}
		return KXTJ2_ERR_SETUP_FAILURE;
	}
//////////////////////////////////////////////
...
static int KXTJ2_InitSelfTest(struct i2c_client *client)
{
	...
	res = hwmsen_read_byte(client, KXTJ2_REG_CTL_REG3, &data);
	if(res != KXTJ2_SUCCESS)
	{
		return res;
	}
//enable selftest bit
	res = hwmsen_write_byte(client, KXTJ2_REG_CTL_REG3,  KXTJ2_SELF_TEST|data);
	if(res != KXTJ2_SUCCESS) //0x2C->BW=100Hz
	{
		return res;
	}
...
/*----------------------------------------------------------------------------*/
int kxtj2_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
//static int kxtj2_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)

static int kxtj2_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
...
static int kxtj2_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sobj.self = obj;
    	sobj.polling = 1;
    	sobj.sensor_operate = kxtj2_gsensor_operate;
...
static int __init kxtj2_init(void)
{
	...
	i2c_register_board_info(1,&kxtj2_i2c_board_info ,1);  //register i2c device
...
{
	GSE_FUN();
//-p	platform_driver_unregister(&kxtj2_gsensor_driver);
}
/*----------------------------------------------------------------------------*/
