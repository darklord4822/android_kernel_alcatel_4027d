...
#define DEBUG 1
/*----------------------------------------------------------------------------*/
//#define CONFIG_BMA222E_LOWPASS   /*apply low pass filter on output*/       
...
#define BMA222E_RETRY_MAX		10

/*----------------------------------------------------------------------------*/

/*----------------------------------------------------------------------------*/
static struct device *compatible_bma222e;   //fangjie add 
static const struct i2c_device_id bma222e_i2c_id[] = {{BMA222E_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_BMA222E={ I2C_BOARD_INFO(BMA222E_DEV_NAME, (BMA222E_I2C_SLAVE_WRITE_ADDR>>1))};
...
/*end of set dataresolution*/
 ...
 /*we set measure range from -2g to +2g in BMA222E_SetDataFormat(client, BMA222E_RANGE_2G), 
                                                    and set 10-bit dataresolution BMA222E_SetDataResolution()*/
...
/*if you changed the measure range, for example call: BMA222E_SetDataFormat(client, BMA222E_RANGE_4G), 
...
static int BMA222E_ReadData(struct i2c_client *client, s16 data[BMA222E_AXES_NUM])
{
	...
	{
		data[BMA222E_AXIS_X] = (s16)buf[BMA222E_AXIS_X*2];
		data[BMA222E_AXIS_Y] = (s16)buf[BMA222E_AXIS_Y*2];
		data[BMA222E_AXIS_Z] = (s16)buf[BMA222E_AXIS_Z*2];
...
static int BMA222E_ReadCalibration(struct i2c_client *client, int dat[BMA222E_AXES_NUM])
{
    ...
    int mul;
    	...
    	mul = obj->reso->sensitivity/bma222e_offset_resolution.sensitivity;
	#endif
...
static int BMA222E_WriteCalibration(struct i2c_client *client, int dat[BMA222E_AXES_NUM])
{
	...
	int lsb; 
	lsb = bma222e_offset_resolution.sensitivity;
...
{
	u8 databuf[2] = {0};    
	int res = BMA222E_SUCCESS;
	struct bma222e_i2c_data *obj = i2c_get_clientdata(client);
	int retry = BMA222E_RETRY_MAX;
	...
	do
	{
		...
		if(hwmsen_read_block(client, BMA222E_REG_POWER_CTL, databuf, 0x01))
		{
		...
		/* confirm the setting */
		databuf[0] = 0xff;
		hwmsen_read_block(client, BMA222E_REG_POWER_CTL, databuf, 0x01);
		if (databuf[0] != databuf[1])
		{
			GSE_ERR("fail to set power mode!\n");
			res = BMA222E_ERR_SETUP_FAILURE;
			continue;
		}
		...
		res = BMA222E_SUCCESS;
		mdelay(20);
	} while ((retry-- > 0) && (res != BMA222E_SUCCESS));

	GSE_LOG("res = %d by retry %d times!\n", res, (BMA222E_RETRY_MAX - retry));
...
static int BMA222E_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	do
	{
		...
		{
			GSE_ERR("bma222e read Dataformat failed \n");
			res = BMA222E_ERR_I2C;
			continue;
		}
		...
		/* confirm the setting */
		databuf[0] = 0xff;
		hwmsen_read_block(client, BMA222E_REG_DATA_FORMAT, databuf, 0x01);
		if (databuf[0] != databuf[1])
		{
...
static int BMA222E_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	do
	{
		...
		{
			GSE_ERR("bma222e read rate failed \n");
			res = BMA222E_ERR_I2C;
			continue;
		}
		...
		/* confirm the setting */
		databuf[0] = 0xff;
		hwmsen_read_block(client, BMA222E_REG_BW_RATE, databuf, 0x01);
		if (databuf[0] != databuf[1])
		{
...
static int bma222e_init_client(struct i2c_client *client, int reset_cali)
{
	...
	res = BMA222E_SetBWRate(client, BMA222E_BW_25HZ);
	if(res != BMA222E_SUCCESS ) 
	{
		return res;
	}
...
static int BMA222E_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if((res = BMA222E_ReadData(client, obj->data))!=0)
	{        
	...
	{
		//printk("raw data x=%d, y=%d, z=%d \n",obj->data[BMA222E_AXIS_X],obj->data[BMA222E_AXIS_Y],obj->data[BMA222E_AXIS_Z]);
		...
		//printk("cvt x=%d, y=%d, z=%d \n",obj->cvt.sign[BMA222E_AXIS_X],obj->cvt.sign[BMA222E_AXIS_Y],obj->cvt.sign[BMA222E_AXIS_Z]);

		//GSE_LOG("Mapped gsensor data: %d, %d, %d!\n", acc[BMA222E_AXIS_X], acc[BMA222E_AXIS_Y], acc[BMA222E_AXIS_Z]);

		//Out put the mg
		//printk("mg acc=%d, GRAVITY=%d, sensityvity=%d \n",acc[BMA222E_AXIS_X],GRAVITY_EARTH_1000,obj->reso->sensitivity);
...
static ssize_t show_regs_value(struct device_driver *ddri, char *buf)
{
	...
	int err = 0, i;
	u8 regsbuf[0x14];
	char strbuf[BMA222E_BUFSIZE] = {0};

	err = hwmsen_read_block(client, 0x00, regsbuf, 0x014);
	if (err != 0)
	{
		GSE_ERR("fail to read regs!\n");
		return 0;
	}
	for (i = 0; i < 0x14; i++)
	{
...
static ssize_t store_power_mode_value(struct device_driver *ddri, char *buf, size_t count)
{
	...
	if((data == 0) ||(data == 1)){
		if(hwmsen_read_block(obj->client, BMA222E_REG_POWER_CTL, &reg_value, 0x01))
		{
			GSE_ERR("read power ctl register err!\n");
			return BMA222E_ERR_I2C;
		}
		...
		{
			reg_value &= ~BMA222E_MEASURE_MODE;
		}
		else/*Suspend Mode*/
		{
			reg_value |= BMA222E_MEASURE_MODE;
		...
		if(hwmsen_write_block(obj->client, BMA222E_REG_POWER_CTL, &reg_value, 0x01))
		{
...
/*----------------------------------------------------------------------------*/
int bma222e_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = BMA222E_SetPowerMode( priv->client, !power_mode);
				}
			}
			break;
...
//static int bma222e_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)

static int bma222e_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
...
/*
	if((err = bma222e_create_attr(&bma222e_gsensor_driver.driver)))
	{
...
*/
	sobj.self = obj;
        sobj.polling = 1;
        sobj.sensor_operate = bma222e_gsensor_operate; //fangjie modify
	...
	//BEGIN: fangjie add for Gsensor compatible design for MMITest G-sensor test. 
	#if 1 //#ifdef SYS_COMPATIBLE
		compatible_bma222e=hwmsen_get_compatible_dev();
		if (device_create_file(compatible_bma222e, &dev_attr_gsensorid) < 0)
		{
			pr_err("Failed to create device file(%s)!\n", dev_attr_gsensorid.attr.name);
		}
		dev_set_drvdata(compatible_bma222e, NULL);
	#endif
	//END: fangjie add for Gsensor compatible design for MMITest G-sensor test. 
...
static int bma222e_i2c_remove(struct i2c_client *client)
{
	int err = 0;	
/*	
	if((err = bma222e_delete_attr(&bma222e_gsensor_driver.driver)))
	{
...
static int __init bma222e_init(void)
{
	GSE_FUN();
	struct acc_hw *hw = bma222e_get_cust_acc_hw();
	i2c_register_board_info(hw->i2c_num, &i2c_BMA222E, 1);
...
{
	GSE_FUN();
//-p	platform_driver_unregister(&bma222e_gsensor_driver);
}
/*----------------------------------------------------------------------------*/
