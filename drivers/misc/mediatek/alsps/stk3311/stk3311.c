...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	else
	{
		...
		{
			idx = obj->fir.idx % STK_FIR_LEN;
			obj->fir.sum -= obj->fir.raw[idx];
			obj->fir.raw[idx] = als_data;
			obj->fir.sum += als_data;
			obj->fir.idx++;
			als_data = obj->fir.sum/STK_FIR_LEN;
		}	
#endif
	}
...
/*----------------------------------------------------------------------------*/
int stk3x1x_write_alsset(struct i2c_client *client, u8 data)
{
	struct stk3x1x_priv *obj = i2c_get_clientdata(client);
	int ret = 0;
    
    ret = stk3x1x_master_send(client, 0x87, &data, 1);
	if(ret < 0)
	{
		APS_ERR("write als = %d\n", ret);
		return -EFAULT;
...
#endif
/*----------------------------------------------------------------------------*/
static void stk3x1x_power(struct stk_alsps_hw *hw, unsigned int on) 
{
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	{
	        //comment by yi.zheng.hz, let it calibrate each time when enable, because some calibration is wrong but it success and don't recalibate
		//obj->psi_set = obj->psi;
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
  ...
  APS_LOG("ALS/PS interrupt pin = %d\n", GPIO_ALS_EINT_PIN);		
	...
	mt_set_gpio_mode(GPIO_ALS_EINT_PIN, EINT2_PS_N_M_EINT); //yi.zheng.hz modify
...
static int stk3x1x_init_client(struct i2c_client *client)
{
	...
	*/
	...
	if((err = stk3x1x_write_alsset(client, 0x60)))
	{
		APS_ERR("write alsset error: %d\n", err);
		return err;
	}	
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
	...
	if(!invalid)
	{
		...
		als_value_old=obj->hw->als_value[idx];
...
#endif
{
	struct stk_alsps_hw *hw = stk_get_cust_alsps_hw();
	struct stk3x1x_i2c_addr addr;
	APS_FUN();
	stk3x1x_power(hw, 1);    
