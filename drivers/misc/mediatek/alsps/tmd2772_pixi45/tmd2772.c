...
//BEGIN: fangjie add for ALSPS (tmd2772 or alsps_ctp_simulate) compatible design for MMITest alsps test. 20140626
#include <linux/hwmsen_dev.h>
static struct device *compatible_alsps;  
//END: fangjie add for ALSPS (tmd2772 or alsps_ctp_simulate) compatible design for MMITest alsps test. 20140626
...
#endif//#if defined(CONFIG_MTK_AUTO_DETECT_ALSPS)
...
/*
		...
		case ALSPS_GET_PS_RAW_DATA_FOR_CALI:
			...
			err = tmd2772_read_data_for_cali(obj->client,&ps_cali);
			if(err)
			{
			   goto err_out;
			}
...
static ssize_t alspsid_show(struct device *dev,
...
static DEVICE_ATTR(alspsid, 0444, alspsid_show, NULL);
static void tmd2772_read_alsps_id_for_compatible(void)
{
		compatible_alsps=hwmsen_get_compatible_dev();
		if (device_create_file(compatible_alsps, &dev_attr_alspsid) < 0)
		{
			pr_err("Failed to create device file(%s)!\n", dev_attr_alspsid.attr.name);
		}
		dev_set_drvdata(compatible_alsps, NULL);
}
#endif
//END: fangjie add for ALSPS (tmd2772 or alsps_ctp_simulate) compatible design for MMITest alsps test. 20140626
...
/*************wwl add psensor get calibration data when calling    end*****************/
	...
	//BEGIN: fangjie add for ALSPS (tmd2772 or alsps_ctp_simulate) compatible design for MMITest alsps test. 20140626
	tmd2772_read_alsps_id_for_compatible();
	//END: fangjie add for ALSPS (tmd2772 or alsps_ctp_simulate) compatible design for MMITest alsps test. 20140626
...
printk("tmd2772_probe");
	...
	//return -1; //fangjie temp test alsps_ctp_simulate function.
